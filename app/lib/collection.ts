...
import { NEXT_PUBLIC_URL, ZORA_COLLECTION_ADDRESS, ZORA_TOKEN_ID_1 } from '../config';
...
export async function getCollection() {
  const collection = await zdk.token({
    token: { address: ZORA_COLLECTION_ADDRESS, tokenId: ZORA_TOKEN_ID_1 },
  ...
  return { name, image, address: ZORA_COLLECTION_ADDRESS, tokenId: ZORA_TOKEN_ID_1 };
}
